...
parser.add_argument('all_queries_fasta_file', type=str)
...
with open(args.queries_csv_file,"r") as queries_csv:
    csv_keys = queries_csv.readline().strip().split(',')
    for line in queries_csv:
        csv_data.append(line.strip())

id_i = csv_keys.index('id')
seq_i = csv_keys.index('sequence')
with open(args.queries_fasta_file,"w") as queries_fasta:
...
csv_keys = []
csv_data = []
with open(args.all_queries_csv_file,"r") as queries_csv:
    csv_keys = queries_csv.readline().strip().split(',')
    for line in queries_csv:
        csv_data.append(line.strip())

cycle_i = csv_keys.index('cycle')
id_i = csv_keys.index('id')
seq_i = csv_keys.index('sequence')
with open(args.all_queries_fasta_file,"w") as queries_fasta:
    for i,csv_i in enumerate(csv_data):
        if i>0: queries_fasta.write('\n')
        csv_i_uscore = csv_i.split(',')
        queries_fasta.write(">")
