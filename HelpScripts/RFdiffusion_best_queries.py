...
ranked_keys = []
best_ids = []
with open(parser.rank_output_csv_file,"r") as ranked_csv:
    ranked_keys = ranked_csv.readline().strip().split(',')
    name_i = ranked_keys.index('name')
    best_i = 0
    for line in ranked_csv:
        if best_i >= parser.FOLD_BEST_WITH_ALPHAFOLD:
            break
        best_ids.append(line.strip().split(',')[name_i])

queries_keys = []
best_queries_rows = []
with open(parser.queries_csv_file,"r") as queries_csv:
...
with open(parser.best_queries_csv_file,"w") as best_queries_csv:
