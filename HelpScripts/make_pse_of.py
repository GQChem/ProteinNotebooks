...
parser = argparse.ArgumentParser(description='Makes pse file from of output')
...
pdb_files = [f for f in os.listdir(args.of_out_folder) if f.endswith(".pdb")]

prot_names=[]
for pdb in pdb_files:
    pdb_file = os.path.join(args.of_out_folder,pdb)
    pdb_name = pdb[:-4]
    cmd.load(pdb_file, pdb_name)
    cmd.do(f"rank_plddt {pdb_name}")
    prot_names.append(pdb_name)
